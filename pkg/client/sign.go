...
func sign(c Client, method, canonicalizedResource string, parmams, headers map[string][]string) {
	...
	if v, ok := parmams["Expires"]; ok {
		expires = true
		date = v[0]
		parmams["KID"] = []string{"sina," + c.accesskey}
	}
	...
	if expires {
		...
		headers["Authorization"] = []string{"SINA " + c.accesskey + ":" + ssig}
	}
}
